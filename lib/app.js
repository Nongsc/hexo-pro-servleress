...
async function createApp(env = process.env) {
  ...
  // EdgeOne serverless：includeFiles 把 www 复制到函数目录的 included_files/www
  if (!fs.existsSync(wwwDir)) {
    wwwDir = path.join(__dirname, 'included_files', 'www');
  }
  if (fs.existsSync(wwwDir)) {
    app.use('/pro', express.static(wwwDir, { redirect: false }));
