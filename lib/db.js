...
class DatabaseManager {
  ...
  // 在 site_config 合并后调用：导入初始管理员用户（幂等，count===0 守卫防重复）
  async ensureInitialUser(config) {
    if (!this.databases) throw new Error('Database not initialized');
    await this._initializeUserAndSettings(this.databases.userDb, this.databases.settingsDb, config || {});
  }
